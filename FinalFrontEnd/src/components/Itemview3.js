...
const Itemview3 = () => {
    ...
    return (
        <section>
            <h1 className='headitem1'>Rice And Curry</h1>
            <div className='ItemTable'>
                {items.length > 0 ? items.map((item) => (
                    <div className='item' key={item.itemId}>
                        <p>{item.name}</p>
                        <p><img className='image' src={`data:image/jpeg;base64,${item.image}`} alt={item.name} /></p>
